...
export default class VariantPicker extends Component {
  ...
  updateVariantPicker(newHtml) {
    ...
    // Dropdown pickers (e.g. product-card variants block) keep their own <select> markup.
    // section-rendering-product-card only returns swatches, so morphing would replace the dropdown.
    if (this.dataset.variantPickerStyle === 'dropdown') {
      const variantJson =
        newHtml.querySelector('variant-picker script[type="application/json"]')?.textContent ??
        newHtml.querySelector(`${this.tagName.toLowerCase()} script[type="application/json"]`)?.textContent;
      const localScript = this.querySelector('script[type="application/json"]');

      if (variantJson && localScript) {
        localScript.textContent = variantJson;
      }

      const sourcePicker = newHtml.querySelector(this.tagName.toLowerCase());
      if (sourcePicker instanceof HTMLElement) {
        const newProductId = sourcePicker.dataset.productId;
        const newProductUrl = sourcePicker.dataset.productUrl;
        ...
        if (newProductUrl) this.dataset.productUrl = newProductUrl;
      }
